...
public class FileManager {
	...
	public boolean createHTML(String fileName, String contents) {
		try {
			...
			if (path.endsWith("/") || path.endsWith("\\")) {
				...
				path += "/" + Constants.ROOT_DIRECOTRY; // + "/" + fileName +
														// ".html";
			}
	...
	public void makeFile(ArrayList<ClassModel> cModels, String projectDetail,
			String homeContents) {
		...
		if (homeContents != null && homeContents.trim().length() > 0) {
			homeContents = links + "<td width='80%'>"
					+ "<h2 class='section-title'>Home</h2>" + homeContents
					+ "</td>";
		...
		for (ClassModel cModel : cModels) {
			...
			if (cModel.getNameLine() != null
				...
				for (PropertyModel prop : cModel.getProperties()) {
					...
					contents += "<td><div class='clsPropertyDeclaration'>"
							+ prop.getNameLine() + "</div>";
					contents += "<div class='clsPropertyDescription'>"
							+ prop.getDescription() + "</div></tr>";
				}
				...
				for (MethodModel method : cModel.getMethods()) {
					contents += "<h2 class='trigger'><input type='button' value='+' style='width:24px' />&nbsp;&nbsp;<a href='#'>"
							...
							+ "</div>"
							+ "<table class='details' rules='all' border='1' cellpadding='6'>"
							+ (method.getAuthor() != "" ? "<tr><th>Author</th><td>"
									+ method.getAuthor() + "</td></tr> "
									: "")
							+ (method.getDate() != "" ? "<tr><th>Date</th><td>"
									+ method.getDate() + "</td></tr> " : "")
							+ (method.getDescription() != "" ? "<tr><th>Description</th><td>"
									+ method.getDescription() + "</td></tr> "
									: "")
							+ (method.getReturns() != "" ? "<tr><th>Returns</th><td>"
									+ method.getReturns() + "</td></tr> "
									: "")
							+ (method.getParams() != null
									&& method.getParams().size() > 0 ? "<tr><th colspan='2' class='paramHeader'>Parameters</td></tr> "
									: "");
			...
			contents = Constants.getHeader(projectDetail) + contents
					+ Constants.FOOTER;
					...
					if (strLine != null && strLine.trim().length() > 0) {
						...
						if (list[0] != null && list[1].trim().length() > 0) {
							if (list[0].equalsIgnoreCase("projectname")) {
								if (list[1] != null
										&& list[1].trim().length() > 0)
									contents += "<h2 style='margin:0px;'>"
											+ list[1] + "</h2>";
							} else {
								if (list[1] != null
										&& list[1].trim().length() > 0)
