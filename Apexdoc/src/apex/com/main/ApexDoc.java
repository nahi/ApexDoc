...
public class ApexDoc {
	...
	private static void printHelp() {
		System.out
				.println("ApexDoc - a tool for generating documentation from Salesforce Apex code class files.\n");
		System.out
				.println("    Invalid Arguments detected.  The correct syntax is:\n");
		System.out
				.println("apexdoc -s <source_directory> [-t <target_directory>] [-h <homefile>] [-a <authorfile>] [-p <scope>]\n");
		System.out
				.println("<source_directory> - The folder location which contains your apex .cls classes");
		System.out
				.println("<target_directory> - Optional. Specifies your target folder where documentation will be generated.");
		System.out
				.println("<homefile> - Optional. Specifies your contents for the home page right panel");
		;
		System.out
				.println("<authorfile> - Optional. Specifies the text file that contains project information for the documentation header.");
		System.out
				.println("<scope> - Optional. Semicolon seperated list of scopes to document.  Defaults to 'global;public'. ");
	}
	...
	private static void fillPropertyModel(PropertyModel propertyModel,
			String name, ArrayList<String> lstComments) {
		...
		for (String comment : lstComments) {
			...
			if (idxStart != -1) {
				propertyModel.setDescription(comment.substring(idxStart + 15)
						.trim());
				inDescription = true;
				continue;
			}
			...
			if (inDescription) {
				...
				if (i < comment.length()) {
					propertyModel.setDescription(propertyModel.getDescription()
							+ ' ' + comment.substring(i));
				}
				continue;
			}
	...
	private static void fillMethodModel(MethodModel mModel, String name,
			ArrayList<String> lstComments) {
		...
		for (String comment : lstComments) {
			...
			if (inDescription) {
				...
				if (i < comment.length()) {
					mModel.setDescription(mModel.getDescription() + ' '
							+ comment.substring(i));
				}
				continue;
			}
	...
	private static void fillClassModel(ClassModel cModel, String name,
			ArrayList<String> lstComments) {
	...
	private static void debug(ClassModel cModel) {
		try {
			...
			if (cModel.getClassName() != null)
				System.out.println(cModel.getClassName());
			if (cModel.getNameLine() != null)
				System.out.println(cModel.getNameLine());
